...
@dataclass
class Transaction:                # 一笔交易，字段与 Chase CSV 列名 100% 对应
    date: date                    # Transaction Date
    post_date: date               # Post Date
    description: str              # Description
    category: str                 # Category
    amount: float                 # Amount（负数=支出，正数=收入/还款）
    tx_type: str                  # Type
    memo: str                     # Memo
...
class Summary:                    # 汇总报告
    ...
    total_spending: float         # 总支出（正数，只含负金额）
    total_credits: float          # 总收入/还款/statement credit（正数，只含正金额）
    by_category: list             # 分类汇总，每项是 [分类名, 笔数, 净金额]
...
def validate_format(fieldnames):
    ...
    if missing:
        print("错误：CSV 缺少必要的列：" + ", ".join(sorted(missing)))
        sys.exit(1)
...
def summarize(transactions):
    ...
    # 把所有负金额取反后相加，得到总支出（正数）
    # round(..., 2) 避免浮点误差，例如 0.1 + 0.2 = 0.30000000000000004
    total_spending = round(sum(-t.amount for t in transactions if t.amount < 0), 2)

    # 把所有正金额相加，得到总还款 / statement credit
...
def _group_by_category(transactions):
    ...
    totals = {}     # key = 分类名，value = 净金额（负=净支出，正=净收入）
    counts = {}     # key = 分类名，value = 笔数

    for t in transactions:          # 支出和收入都记录，不跳过任何一笔
        cat = t.category
        ...
        totals[cat] += t.amount     # 保留原始符号：负=支出，正=还款/credit
        counts[cat] += 1

    # 把字典整理成列表，每项是 [分类名, 笔数, 净金额]
    result = []
    for cat in totals:
        # round 避免浮点精度问题，例如多次累加后出现 -85.30000000000001
        result.append([cat, counts[cat], round(totals[cat], 2)])

    # 按净金额升序排列：最负的（支出最多）排最前面
...
def main():
    ...
    print("=== Chase Credit Card CSV 账单汇总工具 ===")
    print()
